...
function SignInContent() {
  // Password sign-in is the primary/default path -- phone and email OTP
  // are still reachable, just no longer what a visitor lands on first.
  ...
  return (
    <div className="min-h-screen relative flex flex-col items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-[360px] mx-4 bg-white rounded-3xl shadow-2xl p-8">
        {/* Back arrow -- password/email is the landing screen now, so no
            back arrow there; everything else backs up to it. */}
        {(mode === "phone" ||
          mode === "email" ||
        ...
        {/* Divider + alt sign-in method -- Google/Apple sign-in UI removed for
            now (to be set up separately); the only alternative left is
            switching to phone-OTP mode, so this only has something to show
            while in email-OTP mode. */}
        {mode === "email" && (
          <>
